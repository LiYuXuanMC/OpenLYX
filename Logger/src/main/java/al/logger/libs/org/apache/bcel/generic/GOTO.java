...
public class GOTO extends GotoInstruction implements VariableLengthInstruction {
    ...
    public GOTO(final InstructionHandle target) {
        super(al.logger.libs.org.apache.bcel.Const.GOTO, target);
    }
    ...
    public void dump(final DataOutputStream out) throws IOException {
        ...
        if (opcode == al.logger.libs.org.apache.bcel.Const.GOTO) {
            super.dump(out);
        } else { // GOTO_W
            super.setIndex(getTargetOffset());
    ...
    protected int updatePosition(final int offset, final int maxOffset) {
        ...
        if (Math.abs(i) >= Short.MAX_VALUE - maxOffset) { // to large for short (estimate)
            super.setOpcode(al.logger.libs.org.apache.bcel.Const.GOTO_W);
