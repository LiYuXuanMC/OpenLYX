...
import com.reflectmc.reflect.utils.player.MovementUtils;
import com.reflectmc.reflect.utils.player.PlayerUtil;
import com.reflectmc.reflect.utils.rotation.Rotation;
import com.reflectmc.reflect.wrapper.WrapperAdaptArrayList;
...
import com.reflectmc.reflect.wrapper.wrappers.minecraft.entity.InventoryPlayer;
...
public class Scaffold extends Module {
    ...
    public static OptionValue swing = new OptionValue("Swing", true);
    ...
    public Scaffold() {
        ...
        addValues(swing, tower, towerboost, towerTickLen, timer, timerSpeed, render, customPitch, pitch, keepY, sprint, boost, boostY);
    }
        ...
        if (mc.getPlayerController().onPlayerRightClick(thePlayer, mc.getTheWorld(),
            ...
            if (inventory.getStackInSlot(inventory.currentItem()).getItem() != null
                ...
                ItemBlock itemblock = new ItemBlock(inventory
                        .getStackInSlot(inventory.currentItem()).getItem().getWrappedObject());
    ...
    public void onPre(EventPreUpdate event) {
        ...
        if (rotation.getYaw() != 999.0f && rotation.getPitch() != 999.0f) {
            event.setRotation(rotation.getYaw(),rotation.getPitch());
        }
        if (customPitch.getValue())
            event.setRotation(rotation.getYaw(),pitch.getValue().floatValue());
        ...
        if ((tower.getValue()) && isAirUnder(thePlayer) && MovementUtils.isOnGround(1.15)
                && mc.getGameSettings().getKeyBindJump().isKeyDown() && !PlayerUtil.isMovementInput()) {
        ...
        if (MovementUtils.isOnGround(1.15) && mc.getGameSettings().getKeyBindJump().isKeyDown()
                && !PlayerUtil.isMovementInput() && tower.getValue()) {
    ...
    public float[] getRotationsHypixel(BlockPos block, EnumFacing face) {
        EntityPlayerSP thePlayer = mc.getThePlayer();
        final double x = block.getX() + 0.5 - thePlayer.getPosX() + face.getFrontOffsetX() / 2.0;
        final double z = block.getZ() + 0.5 - thePlayer.getPosZ() + face.getFrontOffsetZ() / 2.0;
    ...
    private int getBestSlot(){
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (invalidBlocks.contains(itemBlock.getBlock().getWrappedObject())) continue;
    ...
    public static boolean isValid(final Item item) {
        return ItemBlock.isItemBlock(item) && !invalidBlocks.contains((new ItemBlock(item.getWrappedObject())).getBlock().getWrappedObject());
    }
