...
public class GuiIngameTransformer extends ClassTransformer {
    ...
    public byte[] transform(byte[] bytes) {
        ...
        for (MethodNode method : classNode.methods) {
            ...
            if (method.name.equals(GuiIngame.renderScoreboard.getName()) && method.desc.equals("(L" + Type.getInternalName(ScoreObjective.ScoreObjectiveClass) + ";L" + Type.getInternalName(ScaledResolution.ScaledResolution) + ";)V")) {
                System.out.println("Hook renderScoreboard");
                InsnList drawSc = new InsnList();
                LabelNode L0 = new LabelNode();
                drawSc.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(EventBus.class), "showScoreboard", "()Z"));
                drawSc.add(new JumpInsnNode(IFEQ, L0));
                drawSc.add(new InsnNode(RETURN));
                drawSc.add(L0);
                method.instructions.insert(drawSc);
            }
        }
