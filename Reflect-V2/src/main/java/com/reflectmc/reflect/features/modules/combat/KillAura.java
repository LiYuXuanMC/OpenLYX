...
import com.reflectmc.reflect.utils.timer.TimerUtil;
...
import com.reflectmc.reflect.wrapper.wrappers.minecraft.item.ItemSword;
...
public class KillAura extends Module {
    ...
    public MinMaxValue aps = new MinMaxValue("APS",10D,12D,1D,20D,"0");
    ...
    public MinMaxValue turn = new MinMaxValue("RotationSpeed",114D,120D,5D,180D,"0.0");
    ...
    private final EntitySelector targetSelect = new EntitySelector("Target");
    ...
    public KillAura() {
        ...
        addValues(mode, attackMode, priority, block, aps, range, blockrange, switchDelay, turn, aim, noRotation, targetSelect.getValue(), ESP, throughWallsValue, autoDisable);
    }
    @Override
     ...
     */
    @EventTarget
    public void onUpdate(final EventLivingUpdate event) {
...
//                GL11.glEnable(GL11.GL_DEPTH_TEST);
    ...
    public void onPre(final EventPreUpdate preUpdate){
        ...
        if (curTarget != null && !noRotation.getValue()) { // ROTATION
            double step;
            if (turn.getValue()[0].doubleValue() == turn.getValue()[1].doubleValue())
                step = turn.getValue()[1].doubleValue();
            else
                step = MathUtil.getRandom(turn.getValue()[0].doubleValue(), turn.getValue()[1].doubleValue());
            ...
            cYaw += neededYaw;
            cPitch += neededPitch;
            preUpdate.setRotation(cYaw,cPitch);
        }
    ...
    public void onLoop(final EventLoop event) {
        ...
        double aps;
        if (this.aps.getValue()[0].doubleValue() == this.aps.getValue()[1].doubleValue())
            aps = this.aps.getValue()[1].doubleValue();
        else
            aps = MathUtil.getRandom(this.aps.getValue()[0].doubleValue(), this.aps.getValue()[1].doubleValue());
    ...
    public boolean canBlock() {
        ...
        return block.getValue() != BlockMode.None && Objects.nonNull(curtItem.getWrappedObject()) && ItemSword.isItemSword(curtItem.getItem());
    }
    private void getEntity() {
        ...
        for (Entity entity : mc.getTheWorld().getLoadedEntityList()) {
            if (EntityLivingBase.isEntityLivingBase(entity)){
                ...
                if (isValid(livingBase, range.getValue())){
                    targets.add(livingBase);
                    if (!blockTargetD){
                        blockTarget = livingBase;
                        blockTargetD = true;
    ...
    private boolean isValid(EntityLivingBase entity, double range) {
        EntityPlayerSP thePlayer = mc.getThePlayer();
        if (AntiBot.isEntityBot(new Entity(entity.getWrappedObject())))
            return false;
        ...
        return targetSelect.check(entity);
    }
