...
public class JSR extends JsrInstruction implements VariableLengthInstruction {
    ...
    public JSR(final InstructionHandle target) {
        super(al.logger.libs.org.apache.bcel.Const.JSR, target);
    }
    ...
    public void dump(final DataOutputStream out) throws IOException {
        ...
        if (super.getOpcode() == al.logger.libs.org.apache.bcel.Const.JSR) {
            super.dump(out);
        } else { // JSR_W
            super.setIndex(getTargetOffset());
    ...
    protected int updatePosition(final int offset, final int maxOffset) {
        ...
        if (Math.abs(i) >= Short.MAX_VALUE - maxOffset) { // to large for short (estimate)
            super.setOpcode(al.logger.libs.org.apache.bcel.Const.JSR_W);
