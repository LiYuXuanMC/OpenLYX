...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ)))
        {
            ...
            Rave.getInstance().getEventBus().callEvent(update);
            super.onUpdate();
    ...
    public void onUpdateWalkingPlayer()
    {
        ...
        EventPreUpdate event = new EventPreUpdate(this.isSprinting(),this.isSneaking());
        Rave.getInstance().getEventBus().callEvent(event);
        ...
        EventPreMotion eventPreMotion = new EventPreMotion(this.posX,this.posY,this.posZ,this.rotationYaw,this.rotationPitch,this.onGround);
        Rave.getInstance().getEventBus().callEvent(eventPreMotion);
        if (eventPreMotion.isCanceled())
            return;
        if (eventPreMotion.isValueChanged()){
            this.posX = eventPreMotion.getX();
            this.posY = eventPreMotion.getY();
            this.posZ = eventPreMotion.getZ();
            this.rotationYaw = eventPreMotion.getYaw();
            this.rotationPitch = eventPreMotion.getPitch();
            this.onGround = eventPreMotion.isOnGround();
        }
        ...
        if (this.isCurrentViewEntity())
        {
            ...
            EventPostMotion eventPostUpdate = new EventPostMotion();
            Rave.getInstance().getEventBus().callEvent(eventPostUpdate);
            if (eventPreMotion.isValueChanged()){
                this.rotationYaw = eventPreMotion.getYawOld();
                this.rotationPitch = eventPreMotion.getPitchOld();
            }
        }
    }
    ...
    public void sendChatMessage(String message)
    {
        if (message.startsWith(".")){
            Rave.getInstance().getCommandManager().executeCommand(message);
            return;
        }
    ...
    public void onLivingUpdate()
    {
        ...
        EventNoSlow noSlow = new EventNoSlow();
        Rave.getInstance().getEventBus().callEvent(noSlow);
        if (this.isUsingItem() && !this.isRiding() && noSlow.isSlow())
        {
